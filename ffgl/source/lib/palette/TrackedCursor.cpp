...
TrackedCursor::TrackedCursor(Palette* palette_, std::string cid, std::string cidsource, Layer* layer_, glm::vec2 pos_, float z) {
	// _area = area_;
	_palette = palette_;
	// _last_pitches.clear();
	// _last_channel = -1;
	...
	if ( Debug) {
		NosuchDebug("NEW TrackedCursor layer=%d cid=%s source=%s",_layer, cid.c_str(),_cidsource.c_str());
	}
...
void TrackedCursor::advanceTo( int tm )
{
	...
	int smoothxyz_factor = 1 + layer()->params.smoothxyz;
