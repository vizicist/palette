...
func (quad *Quad) onCursorEvent(state ActiveCursor) error {
	...
	// Real input turns attract mode off, but not on the first touch: it takes a
	// few of them close together (see noticeTouch), so that someone brushing
	...
	if !state.Current.IsAttractGenerated() {
		...
		theAttractManager.noteAttractActivity()
	}
