...
func dcompare(d1 Datum, d2 Datum) int {
	...
	if d1.dtype == D_NUM && d2.dtype == D_NUM {
		...
		if isnoval(d1) == isnoval(d2) {
			n = 0
		} else {
			n = 1
		}
	} else if d1.dtype == D_DBL || d2.dtype == D_DBL {
		dbl1 := dblval(d1)
		dbl2 := dblval(d2)
		if dbl1 > dbl2 {
			n = 1
		...
		/* hence this hack. */
		if d1.dtype == D_ARR || d1.dtype == D_OBJ {
			d1 = Nullval
		}
		if d2.dtype == D_ARR || d2.dtype == D_OBJ {
			d2 = Nullval
		}
		lng1 := numval(d1)
		lng2 := numval(d2)
