...
func funcnameof(BYTEFUNC i) string {
...
func taskptr(tid int) Ktaskp {
	h := hashtable(Tasktable,numdatum(tid),H_LOOK)
	if h != nil {
		return h.val.u.(Ktaskp)
	} else {
		return nil
	}
}
...
func makesuredefined(dpp **Datum,nm string) {
	fd := **dpp;
	if ( isnoval(fd) ) {
		s := lookup(uniqstr(nm))
		loadsym(s,0);
...
exectasks(int nosetjmp)
{
	...
	defer func() {
        if r := recover(); r != nil {
            log.Println("Recovered in exectasks!  r=%v\n", r)
        }
    }()
	// 	setjmp(Begin);
