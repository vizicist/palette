...
func (quadpro *QuadPro) Start() {
	...
	if err != nil {
		...
		err = quadpro.Load(bytes, "quad", "_Current.json")
		LogIfError(err)
	}
}
...
func (quadpro *QuadPro) onCursorEvent(state ActiveCursor) error {
	...
	// See if this Cursor was generated by Attract mode
	isAttractGenerated := state.Current.IsAttractGenerated()

	// Any non-attract-generated cursor or Button will turn attract mode off.
	if !isAttractGenerated && TheAttractManager.AttractModeIsOn() {
		TheAttractManager.SetAttractMode(false)
	}
	if !isAttractGenerated {
...
func (quadpro *QuadPro) Load(bytes []byte, category string, filename string) error {
	...
	case "engine":
		...
		if !isCurrent {
			err = EngineParams.Save("engine", "_Current")
		}
	case "quad":
		if !isCurrent {
