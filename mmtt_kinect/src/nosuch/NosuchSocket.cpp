...
NosuchSocket::NosuchSocket()
{
	...
	mu32_Tick64Hi     = 0;
	ms64_MaxIdleTime  = 0;
	mu32_EventTimeout = 1000;
}

NosuchSocket::~NosuchSocket()
{
	// NosuchDebug("NosuchSocket::cSocket  DESTRUCTOR\n");
	if (mb_Initialized) 
	{
		Close();
		WSACleanup();
...
DWORD NosuchSocket::Initialize()
{
	// NosuchDebug("NosuchSocket::NosuchSocket  Initialize\n");
	if (mb_Initialized)
		return 0;

	if ( NosuchNetworkInit() ) {
		NosuchDebug("Unable to initialize networking in NosuchSocket::Initialize!?");
		return 1;
	}

	WSADATA wsaData;
	DWORD err = WSAStartup(MAKEWORD(1,1), &wsaData);
	if (err) {
		NosuchDebug("NosuchSocket::Initialized failed in WSAStartup!? err=%d",err);
		return err;
	}
...
DWORD NosuchSocket::Close()
{
	// NosuchDebug("NosuchSocket::NosuchSocket  Close\n");
...
NosuchSocket::eState NosuchSocket::GetState()
{
	return mi_List.me_State;
}

// Get the count of open sockets
DWORD NosuchSocket::GetSocketCount()
{
	return mi_List.mu32_Count;
}

// returns all open sockets: Key = socket handle, Value = peer IP
DWORD NosuchSocket::GetAllConnectedSockets(cHash<SOCKET,DWORD>* pi_SockList)
{
	#if TRACE_EVENTS || TRACE_LOCK
...
DWORD NosuchSocket::CreateSocket()
{
...
DWORD NosuchSocket::Listen(DWORD u32_BindIP, USHORT u16_Port, DWORD u32_EventTimeout, DWORD u32_MaxIdleTime)
{
	// NosuchDebug("NosuchSocket::NosuchSocket  Listen\n");
	#if TRACE_EVENTS || TRACE_LOCK
		TraceA("Listen()");
	#endif
	...
	NosuchSocketConnection* pk_Data = &mi_List.mk_Data[0];
...
// *************************** ATTENTION ************************************
DWORD NosuchSocket::ConnectTo(DWORD u32_ServIP, USHORT u16_Port, DWORD u32_EventTimeout, DWORD u32_MaxIdleTime)
{
	// NosuchDebug("NosuchSocket::NosuchSocket  ConnectTo  ServIP=0x%lx\n",u32_ServIP);
	#if TRACE_EVENTS || TRACE_LOCK
		TraceA("ConnectTo()");
	#endif
	...
	pk_Data->u16_Port_dest = u16_Port;
	pk_Data->u16_Port_source = 0;
	return 0;
}

// This function is for use on a server only
// It can be called to force a disconnect of a specific client from the server
DWORD NosuchSocket::DisconnectClient(SOCKET h_Socket)
{
	// NosuchDebug("NosuchSocket::NosuchSocket  DisconnectClient\n");
	#if TRACE_EVENTS || TRACE_LOCK
...
DWORD NosuchSocket::ProcessEvents(DWORD*   pu32_Events,       // OUT
                                  DWORD*   pu32_IP,           // OUT
                                  USHORT*  pu16_Port_source,  // OUT
								  SOCKET*  ph_Socket,         // OUT
								  NosuchSocketConnection**   ph_connection,
                                  NosuchSocketMemory** ppi_RecvMem, // OUT
                                  DWORD*   pu32_Read,         // OUT
								  DWORD*   pu32_Sent)         // OUT
{
	// NosuchDebug("NosuchSocket::NosuchSocket  ProcessEvents\n");
	#if TRACE_EVENTS || TRACE_LOCK
		TraceA("Entering ProcessEvents()");
	#endif

	DWORD u32_Error;
	NosuchSocketConnection* conn;
	...
	*ppi_RecvMem = 0;
	...
	if (!mi_List.mu32_Count) {
		// NosuchDebug("Returning WSAENOTCONN, mu32_count = 0?\n");
		return WSAENOTCONN; // No socket open
	}
	...
	conn = &mi_List.mk_Data[u32_Index];

	// -------------------------------------------

	// Get the event(s) that occurred and their associated error array
	if (WSAEnumNetworkEvents(conn->h_Socket, mi_List.mh_Events[u32_Index+1], &k_Events) == SOCKET_ERROR)
	{
		u32_Error = WSAGetLastError();
	...
	// After shutdown(conn->h_Socket) has been called remove all events except FD_CLOSE
	if (conn->b_Shutdown) k_Events.lNetworkEvents &= FD_CLOSE;

	// Signal the FD_TIMEOUT flag always together with FD_CLOSE
	if (conn->b_Timeout && k_Events.lNetworkEvents & FD_CLOSE) 
		k_Events.lNetworkEvents |= FD_TIMEOUT;

	#if TRACE_EVENTS
		char s8_Buf[200];
		FormatEvents(k_Events.lNetworkEvents, s8_Buf);
		TraceA("# Socket %X: Events: %s", conn->h_Socket, s8_Buf);
	#endif
	...
	*pu32_Events = k_Events.lNetworkEvents;
	...
	if (k_Events.lNetworkEvents & FD_ACCEPT)
	{
		...
		SOCKET h_Socket = WSAAccept(conn->h_Socket, (LPSOCKADDR)&k_Addr, &s32_Len, (LPCONDITIONPROC) AcceptCondition, (DWORD_PTR)this);
		if (h_Socket == INVALID_SOCKET)
		{
			u32_Error = WSAGetLastError();
			goto _Exit;
		...
		conn = mi_List.Add(h_Socket, h_Event);
		conn->parent = this;
		// Store the client's IP in the socket list
		conn->u32_IP = k_Addr.sin_addr.s_addr;
		conn->u16_Port_source = k_Addr.sin_port;
		// Store the time when the last action was executed on the socket
		conn->s64_IdleSince = GetTickCount64();

		conn->_source = ip_port_source(conn->u32_IP,conn->u16_Port_source);
	...
	*ph_connection = conn;
	*ph_Socket = conn->h_Socket;
	*pu32_IP   = conn->u32_IP;
	*pu16_Port_source   = conn->u16_Port_source;
	...
	if (k_Events.lNetworkEvents & FD_CONNECT)
	{
		if (k_Events.iErrorCode[FD_CONNECT_BIT])
		{
			...
			NosuchDebug("NosuchSocket event FD_CONNECT error = %d\n",u32_Error);
			goto _Exit;
		}

		// Store the time when the last action was executed on the socket
		conn->s64_IdleSince = GetTickCount64();
		NosuchDebug("NosuchSocket event FD_CONNECT good!\n");
	...
	if (k_Events.lNetworkEvents & FD_READ)
	{
		...
		if (WSARecv(conn->h_Socket, &k_Buf, 1, pu32_Read, &u32_Flags, 0, 0) == SOCKET_ERROR)
		{
			u32_Error = WSAGetLastError();
		...
		if (!conn->pi_RecvMem)
			 conn->pi_RecvMem = new NosuchSocketMemory(MEMORY_INITIAL_SIZE);
		
		conn->pi_RecvMem->Append(ms8_ReadBuffer, *pu32_Read);

		// Store the time when the last action was executed on the socket
		conn->s64_IdleSince = GetTickCount64();

		*ppi_RecvMem = conn->pi_RecvMem;
	...
	if (k_Events.lNetworkEvents & FD_WRITE)
	{
		...
		if (conn->s8_SendBuf)
		{
			...
			DWORD u32_Before = conn->u32_SendPos;
			u32_Error = SendDataBlock(conn->h_Socket, conn->s8_SendBuf, &conn->u32_SendPos, conn->u32_SendLen);
			*pu32_Sent = conn->u32_SendPos - u32_Before;

			if (conn->u32_SendPos == conn->u32_SendLen)
			{
				// All data has been sent successfully -> delete the buffer
				delete conn->s8_SendBuf;
				conn->s8_SendBuf = 0;
			}
...
DWORD NosuchSocket::WSAWaitForMultipleEventsEx(DWORD u32_Count, DWORD* pu32_Index, WSAEVENT* ph_Events, DWORD u32_Timeout)
{
...
#ifdef ORIGINAL
int WINAPI NosuchSocket::AcceptCondition(WSABUF* pk_CallerId, WSABUF* pk_CallerData, QOS* pk_SQOS, QOS* pk_GQOS, 
										 WSABUF* pk_CalleeId, WSABUF* pk_CalleeData, UINT* pu32_Group, DWORD_PTR p_Param)
#endif

int WINAPI NosuchSocket::AcceptCondition(LPWSABUF pk_CallerId, LPWSABUF pk_CallerData, LPQOS pk_SQOS, LPQOS pk_GQOS, 
										 LPWSABUF pk_CalleeId, LPWSABUF pk_CalleeData, GROUP * pu32_Group, DWORD_PTR p_Param)
{
	NosuchSocket* p_This = (NosuchSocket*)p_Param;
...
DWORD NosuchSocket::ProcessIdleSockets(char* s8_Caller)
{
	if (!ms64_MaxIdleTime)
		return 0;
	...
	for (DWORD i=0; i<mi_List.mu32_Count; i++)
	{
		NosuchSocketConnection* conn = &mi_List.mk_Data[i];

		if (!conn->u32_IP || conn->b_Shutdown)
		...
		if (s64_Now - conn->s64_IdleSince + 100 >= ms64_MaxIdleTime)
		{
			#if TRACE_EVENTS
				TraceA("+ Shutting down idle Socket %X", conn->h_Socket);
			#endif

			// Set FD_TIMEOUT flag when FD_CLOSE will be fired
			conn->b_Timeout = TRUE;
			// Ignore all events on this socket except FD_CLOSE
			conn->b_Shutdown = TRUE;

			// DO NOT call closesocket() here !!!
			// shutdown() --> FD_CLOSE --> b_Shutdown=TRUE --> cList::Remove() --> closesocket()
			shutdown(conn->h_Socket, SD_BOTH);
		}
		else if (s64_Oldest > conn->s64_IdleSince)
		{
			// Get the oldest "last activity"
			s64_Oldest = conn->s64_IdleSince;
		}
	}
...
DWORD NosuchSocket::SendDataBlock(SOCKET h_Socket, char* s8_Buf, DWORD* pu32_Pos, DWORD u32_Len)
{
...
DWORD NosuchSocket::SendTo(SOCKET h_Socket, char* s8_Buf, DWORD u32_Len)
{
	#if TRACE_EVENTS || TRACE_LOCK
	...
	NosuchSocketConnection* conn = &mi_List.mk_Data[s32_Index];
	if (conn->b_Shutdown || conn->b_Closed)
	...
	DWORD u32_Pos = 0;
	// NosuchDebug("SendTo, this=%ld h_Socket=%ld",(long)this,(long)h_Socket);
	...
	if (u32_Error == WSAEWOULDBLOCK)
	{
		...
		conn-> s8_SendBuf = new char[u32_Len];
		conn->u32_SendLen = u32_Len;
		conn->u32_SendPos = 0;
		memcpy(conn->s8_SendBuf, s8_Buf, u32_Len);
		return u32_Error;
	}
...
DWORD NosuchSocket::GetLocalIPs(cHash<DWORD,DWORD>* pi_IpList)
{
	pi_IpList->Clear();
...
LONGLONG NosuchSocket::GetTickCount64()
{
...
void NosuchSocket::FormatEvents(DWORD u32_Events, char* s8_Buf)
{
	s8_Buf[0] = 0;
...
void NosuchSocket::TraceA(const char* s8_Format, ...)
{
...
NosuchSocketConnection* NosuchSocket::cList::Add(SOCKET h_Socket, HANDLE h_Event)
{
	// Store max 63 sockets
	if (mu32_Count >= WSA_MAXIMUM_WAIT_EVENTS-1)
		return 0;

	memset(&mk_Data[mu32_Count], 0, sizeof(NosuchSocketConnection));
...
void NosuchSocket::cList::RemoveAll()
{
	while (mu32_Count)
	{
		Remove(mu32_Count-1);
...
void NosuchSocket::cList::RemoveClosed()
{
...
BOOL NosuchSocket::cList::Remove(DWORD u32_Index)
{
...
int NosuchSocket::cList::FindSocket(SOCKET h_Socket)
{
...
DWORD NosuchSocket::cLock::Request(kLock* pk_Lock)
{
...
DWORD NosuchSocket::cLock::Loop(kLock* pk_Lock)
{
	DWORD u32_Error = pk_Lock->Init();
	if (u32_Error)
		return u32_Error;
...
****************************************************************************************
	...
	embedded class NosuchSocketMemory
...
NosuchSocketMemory::NosuchSocketMemory(DWORD u32_InitialSize)
{
	 ms8_Mem  = new char[u32_InitialSize];
	mu32_Size = u32_InitialSize;
	mu32_Len  = 0;
}
NosuchSocketMemory::~NosuchSocketMemory()
{
	delete ms8_Mem;
}
char* NosuchSocketMemory::GetBuffer()
{
	return ms8_Mem;
}
DWORD NosuchSocketMemory::GetLength()
{
	return mu32_Len;
}

// Append data to the end. If more memory is required, the current size is doubled.
void NosuchSocketMemory::Append(char* s8_Data, DWORD u32_Count)
{
...
void NosuchSocketMemory::DeleteLeft(DWORD u32_Count)
{
