...
CBlobContour& CBlobContour::operator=( const CBlobContour &source )
{
	...
	{		
		...
		_area = source._area;
		_perimeter = source._area;
		_moments = source._moments;
	}
	return *this;
}
...
double CBlobContour::GetPerimeter()
{
	...
	_perimeter = cvContourPerimeter( GetContourPoints() );
...
double CBlobContour::GetArea()
{
	...
	_area = fabs( cvContourArea( GetContourPoints() ));
...
double CBlobContour::GetMoment(int p, int q)
{
	...
	if( _moments.m00 == -1)
	{
		cvMoments( GetContourPoints(), &_moments );
	}
		
	return cvGetSpatialMoment( &_moments, p, q );
